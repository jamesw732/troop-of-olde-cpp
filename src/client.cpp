...
int main()
{
    ...
    auto skybox = world.entity();
    skybox.set<Color>(BLACK);
    skybox.set<ModelPointer>({&loaded_models["cube"].model});
    skybox.add<RenderPosition>();
    skybox.add<RenderRotation>();
    skybox.set<Scale>({{500, 500, 500}});
